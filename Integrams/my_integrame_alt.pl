...
my_sols(integ(H, W, L, Vocab), Sols, L_qs) :- intrebari(integ(H, W, L, Vocab), L_qs),
                                              cauta_cuvinte(integ(H, W, L, Vocab), L_qs, Sols).
...
cauta_solutii(_, [], _, []).
cauta_solutii(integ(H, W, L, Vocab), 
              [((R, C), Q, Dir, Id) | Ls],
              L_qs, 
              [(Q, X) | Curr_Sol]) :- findall((Q, P1, Q2, P2), my_intersect(integ(H, W, L, Vocab), L_qs, Q, P1, Q2, P2), L_int),
                                      parcurge_spatiu(integ(H, W, L, Vocab),
                                                      ((R, C), Q, Dir, Id),
                                                      Lung),
                                      verif_posibil_rasp(Lung, Vocab, Rasp), !,
                                                                    cauta_solutii(integ(H, W, L, Vocab), Ls, L_qs, Curr_Sol),
                                                                    member(X, Rasp), \+member((_, X), Curr_Sol),
                                                                    verif_valid_sol(Q, X, L_int, Curr_Sol).

rezolvare(W, Solutie) :- intrebari(W, L_qs),
